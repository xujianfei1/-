...
/**
 * 切换 banned 状态 (admin 用).
 * 返回更新后的 banned. 不允许 ban 最后一个 admin (防误操作锁死自己).
 */
export async function setUserBanned(userId: string, banned: boolean) {
  if (banned) {
    ...
    if (target?.isAdmin) {
      // 数一下还有几个 admin, 如果 ban 之后剩 0 个就拒绝
      const adminCount = await prisma.user.count({
        where: { isAdmin: true, banned: false },
      });
      // adminCount 包含目标自己 (还没 ban). ban 之后剩 adminCount - 1
      if (adminCount <= 1) {
        throw new Error('不能 ban 最后一个 admin, 会锁死系统');
      }
    }
  }
  return prisma.user.update({
